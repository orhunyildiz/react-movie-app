...
function Star({ fill, starIndex, onRating, color, size, onHoverEnter, onHoverLeave }) {
    const handleClick = (e) => {
        const { left, width } = e.currentTarget.getBoundingClientRect();
        const clickX = e.clientX - left;
        if (clickX < width / 2) {
            onRating(starIndex - 0.5);
        } else {
            onRating(starIndex);
        }
    };
    ...
    const handleMouseMove = (e) => {
        ...
        if (hoverX < width / 2) {
            onHoverEnter(starIndex - 0.5);
        } else {
            onHoverEnter(starIndex);
        }
    };
    ...
    return (
        ...
        >
            {fill === 1 ? (
                ...
                // empty star
                <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} fill="" viewBox="0 0 16 16">
